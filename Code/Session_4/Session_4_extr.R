...
# convert to factors
pos_dat$Pop <- factor(pos_dat$Pop)
pos_dat$sex <- factor(pos_dat$sex)
...
factor1a 	<- c(rep("control", 5), rep("treated", 5), rep("control", 5), rep("treated", 5))
factor2a <- c(rep("low", 10), rep("high", 10))
factor1 <- factor(factor1a)
factor2 <- factor(factor2a)
...
fact1a <- c(rep("Control", 12), rep("Treat", 12))
fact2a <- c(rep("Low", 6), rep("High", 6), rep("Low", 6), rep("High", 6))
fact1 <- factor(fact1a)
fact2 <- factor(fact2a)
