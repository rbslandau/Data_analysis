...
# load data
data(varechem)
# reduce to chemical data
varechem_red <- varechem[ ,1:11]
...
chisq.plot(varechem_red, quan = 1, ask = FALSE)
...
## ----data_transform_sqrt, include = TRUE, eval = TRUE, echo = TRUE, purl = TRUE-----------------------------------------
varechemical <- sqrt(varechem_red)
...
chisq.plot(varechemical, quan = 1, ask = FALSE)
mtext( "Soil data")
...
soil_scaled <- scale(varechemical)
...
set.seed(1001) 
estim_ncpPCA(soil_scaled_dfna, method.cv = "Kfold", pNA = 0.20, ncp.max = 6, verbose = FALSE)
...
# make example reproducible
set.seed(100) 
estim_ncpPCA(soil_scaled_dfna, method.cv = "gcv", ncp.max = 6, verbose = FALSE)
...
num_col_soilsdata <- ncol(varechemical)
...
cor(scale(varechemical), pca_scores)
...
k.max <- 2
# run function
oTPO <- opt.TPO(soil_scaled_df, k.max = k.max)
oTPO$pc.noord$lambda
...
spc <- sPCAgrid(soil_scaled_df, k = k.max, lambda = oTPO$pc.noord$lambda)
...
pca_index <- c(1:ncol(varechemical))
# extract scores
